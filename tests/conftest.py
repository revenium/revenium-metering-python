...
from revenium_metering import ReveniumMetering, DefaultAioHttpClient, AsyncReveniumMetering
from revenium_metering._utils import is_dict
...
logging.getLogger("revenium_metering").setLevel(logging.DEBUG)
...
def client(request: FixtureRequest) -> Iterator[ReveniumMetering]:
    strict = getattr(request, "param", True)
    if not isinstance(strict, bool):
        raise TypeError(f"Unexpected fixture parameter type {type(strict)}, expected {bool}")

    with ReveniumMetering(base_url=base_url, api_key=api_key, _strict_response_validation=strict) as client:
        yield client


@pytest.fixture(scope="session")
async def async_client(request: FixtureRequest) -> AsyncIterator[AsyncReveniumMetering]:
